...
const Documents: React.FC = () => {
  const renderMarkdown = (markdown: string) => {
    ...
    return lines.map((line, index) => {
      if (line.startsWith('# ')) {
        return <h1 key={index} className="text-3xl font-bold mt-4 text-[#0086AD]">{line.slice(2)}</h1>;
      } else if (line.startsWith('## ')) {
        return <h2 key={index} className="text-2xl font-semibold mt-4 text-[#0086AD]">{line.slice(3)}</h2>;
      } else if (line.startsWith('- ')) {
